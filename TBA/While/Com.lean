...
inductive Com where
  ...
  -- `while` is already a keyword, so escape name using French quotes «...».
  -- Note that in *expression contexts* (basically everywhere except at `induction/cases ... | «while» ...`),
  -- `Com.while` or often simply `.while` is accepted without escaping.
  | «while» (b : Expr) (c : Com)
